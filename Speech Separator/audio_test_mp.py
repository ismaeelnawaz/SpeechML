...
# dir for test audio
# data_dir = 'Testing/mix.wav'

# not useful during sample test
sum_dir = 'sum'
# dir to load model
#train_dir = os.path.join('log_12000', 'model.ckpt-10400')
train_dir = os.path.join('/media/ismaeel/PC/train-1/GC_code/log_12000', 'model.ckpt-10000')
...
def record_audio(record_seconds=10, sampling_rate=SAMPLING_RATE):
    CHUNK = 1024
    FORMAT = pyaudio.paFloat32
    CHANNELS = 1
    RATE = sampling_rate
    # assert(args.record_seconds == 0)
    # assert 0 == 1, "Record Settings should not be 0. Please provide another value."
...
def play_audio(audio):
    ...
    while(z + FRAME_SIZE < len(audio)):
        frames.append(audio[z:z+FRAME_SIZE])
        z = z + FRAME_SIZE
    
    for i in range(len(frames)):
            stream.write(frames[i].tostring())
...
def out_put(N_frame, dir, batch_size=5, input_audio=False, ratio_mask=False, frames=None, CONFUSION_MATRIX=True, silence_mask=True, visualize_embedding=False, visualize_tsne =False, spectrograms=False, time_domain_graphs=True):
    ...
    #librosa.output.write_wav(os.path.join(dir,'SEN_1.wav'), out_audio1 * (1/np.max(out_audio1)), SAMPLING_RATE)
    #librosa.output.write_wav(os.path.join(dir,'SEN_2.wav'), out_audio2 * (1/np.max(out_audio2)), SAMPLING_RATE)
    ...
    #sf.write(os.path.join(dir,'mix_mp.wav'), mix * (1/np.max(mix)), SAMPLING_RATE)
    # librosa.output.write_wav(os.path.join(dir,'out_1.wav'), out_audio
...
if __name__ == '__main__':
    ...
    # audio = record_audio(sampling_rate=rec_sample_rate)
    # librosa.output.write_wav(os.path.join('/home/usman/repos/train-1/Testing', '44k.wav'), audio, rec_sample_rate)
    # resampling_factor = rec_sample_rate/SAMPLING_RATE
    # audio_8k = resample(audio, int(len(audio)/resampling_factor))
    # librosa.output.write_wav(os.path.join('/home/usman/repos/train-1/Testing', '8k.wav'), audio_8k, SAMPLING_RATE)
    # print("Duration of original audio", len(audio)/rec_sample_rate)
    # print("Duration of new audio", len(audio_8k)/SAMPLING_RATE)
    # print("Audio resampled at frequency", len(audio)//resampling_factor)

    # audio1,_ = librosa.load('Testing/BBC/news2.wav', sr=44100)
    # audio2,_ = librosa.load('Testing/BBC/sairbeen2.wav', sr=44100)
    # resampling_factor = rec_sample_rate/SAMPLING_RATE
    # audio1_8k = resample(audio1, int(len(audio1)/resampling_factor))
    # audio2_8k = resample(audio2, int(len(audio2)/resampling_factor))
    # audio_8k = audio1_8k[:min(len(audio1_8k), len(audio2_8k))] + audio2_8k[:min(len(audio1_8k), len(audio2_8k))]
    audio_8k = record_audio(record_seconds=5)
    play_audio(audio_8k)

    # start_time = time.time()
    out_put(400, os.path.join('/home/ismaeel/Documents'), ratio_mask=False, silence_mask=False, CONFUSION_MATRIX=False, input_audio=True, frames=audio_8k)
