...
class SpeechEnhancer(QMainWindow, QWidget):
    ...
    def run(self):
        if 'exportpath' in globals() and os.path.exists(exportpath) and os.path.exists(filename):            
            ...
            app.processEvents()
            #out_put(400, os.path.join(exportpath), input_audio=True, frames=audio_8k, ratio_mask=False, CONFUSION_MATRIX=False, silence_mask=False, visualize_embedding=False, visualize_tsne =False, spectrograms=False, time_domain_graphs=False)
