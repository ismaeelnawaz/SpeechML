...
# train_dir = os.path.join('log_12000', 'model.ckpt-12000')
train_dir = os.path.join('/media/ismaeel/PC/train-1', 'model.ckpt-9000')
...
NUMBER_OF_SAMPLES = 25   # Samples for audio testing
...
def out_put(N_frame, dir, dirSave):
    ...
    with tf.Graph().as_default():
        ...
        data_generator = AudioSampleReader(os.path.join(dir))
        ...
        print('Writing Files')
        mix, _ = librosa.load(os.path.join(dir), 8000)
        length = len(mix)

        librosa.output.write_wav(os.path.join(dirSave,'OutputAudio1.wav'), out_audio1, SAMPLING_RATE)
        librosa.output.write_wav(os.path.join(dirSave,'OutputAudio2.wav'), out_audio2, SAMPLING_RATE)
